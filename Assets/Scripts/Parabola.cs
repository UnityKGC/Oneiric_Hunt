...
public class Parabola : MonoBehaviour
{
    ...
    private GameObject _obj; // 이동하는 객체

    private float _time; // 이동 시간
    public void SetValue(GameObject obj, Vector3 targetPos, float time) // Object가 targetPos위치로 time초 동안 포물선 형태로 이동한다
    {
        _obj = obj;
    ...
    IEnumerator StartParabola()
    {
        ...
        distanceXz.y = 0f; // y는 후에 따로 구하므로 0으로 초기화시킴.
        ...
        float Vy = (sY / _time) + (0.5f * Mathf.Abs(Physics.gravity.y) * _time); // 시간에 따른 기존 Y값과, 중력적용한 y값을 더하여, 오브젝트의 현재 위치를 고려한 포물선 계산을 함.
